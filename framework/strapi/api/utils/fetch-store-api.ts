...
import { StrapiApiError, StrapiNetworkError } from './errors'
...
export default async function fetchStoreApi<T>(
  ...
  try {
    ...
    throw new StrapiNetworkError(`Fetch to Strapice failed: ${error.message}`)
  }
  ...
  if (!res.ok) {
    ...
    throw new StrapiApiError(msg, res, data)
  }

  if (res.status !== 204 && !isJSON) {
    throw new StrapiApiError(
      `Fetch to Strapice API failed, expected JSON content but found: ${contentType}`,
      res
    )
  }
